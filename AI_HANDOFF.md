...
- 首页晚晚聊天输入区新增暗色玻璃麦克风按钮；点击开始/再点结束，录音时显示红色呼吸波纹和「晚晚在听…」状态。
- 前端用 `getUserMedia({ audio:true }) + MediaRecorder` 录制 WebM/Opus，限制 60 秒和约 10MB；识别成功后先回填输入框，再调用原有 `submitAiRadioMessage()` 自动发送，语音来源消息显示小麦克风标记。
- AI 设置新增「语音识别」分组，支持硅基流动与自定义 OpenAI 兼容 Base URL/Key/model；配置仅保存在 localStorage `mineradio-ai-asr-config-v1`。
- `server.js` 新增 `POST /api/ai/transcribe`：接收 raw audio body，12MB 上限，用内存 Buffer + `Blob/FormData` 转发 ASR，30 秒超时，结构化返回 `ASR_KEY_REQUIRED` / `ASR_BODY_TOO_LARGE` / `ASR_TIMEOUT` / `ASR_UPSTREAM_FAILED` / `ASR_EMPTY` 等错误。
- `desktop/main.js` 已为主窗口本地页面配置 media 权限检查，允许音频录制并保留原有视频/摄像头能力。拒绝权限、无设备、设备被占用、超时、空识别都有中文提示并会恢复 UI busy 状态。
- 验证完成：`node --check server.js`、`node --check desktop/main.js`、前端内联脚本解析、尾随空白/冲突标记扫描通过；本地路由已验证缺 Key、空音频、12MB 超限、错误 Key 401、自定义端点不可达的结构化错误。页面 DOM 确认麦克风按钮和两类 ASR 设置已加载，项目目录没有新音频文件。当前 macOS 工作区副本不含 `.git`，因此 `git diff --check` 无法执行，已用等价文本扫描补位。
- 已知限制：第一版没有 TTS，也没有按住说话/松开发送；自定义端点需实现 OpenAI `/v1/audio/transcriptions` 兼容响应。

### 2026-07-10

- 首页“施工中”主卡片已替换为「晚晚的音乐电台」文字聊天界面，保留“展开播放器控制台”按钮。
- 新增与全局暗色玻璃风格一致的 AI 设置弹窗；DeepSeek Key 使用密码输入框并仅保存在当前设备 localStorage，不写入项目文件。
- `server.js` 新增 `POST /api/ai/chat`：调用 DeepSeek JSON 模式，结合当前歌曲、后续队列、最近对话和最近播放生成回复；推荐歌曲通过网易云搜索优先、QQ 音乐兜底解析成 Mineradio 原生歌曲对象。
- AI 推荐歌曲按原顺序插入当前歌曲之后并立即播放第一首，推荐播放完后继续原队列；第一版没有接入 TTS。
- AI 电台已接入 Home 现有 Open-Meteo 天气状态；聊天前自动刷新超过 15 分钟的天气，并向 DeepSeek 传入城市、天气、温度、体感、湿度、风速和天气情绪。询问天气时要先回答真实天气再选曲，接口不可用时禁止猜测。
- 已运行 `node --check server.js` 和内联脚本解析检查；浏览器验证首页、AI 设置弹窗、未配置 Key 的接口提示和整体视觉正常。
...
### 2026-07-16 - AI 电台天气位置设置

- `public/index.html` 的“AI 音乐电台设置”新增“天气位置”分组，支持输入/选择城市以及通过现有 `/api/weather/ip-location` 自动定位城市。
- 城市继续保存在 `localStorage` 的 `mineradio-weather-city`，不新增位置文件或服务端持久化数据。
- 保存新城市后会立即清空旧天气上下文并调用 `/api/weather/radio` 刷新；后续 `/api/ai/chat` 继续通过现有 `context.weather` 携带所选城市的实时天气。
- 已验证 `/api/weather/radio?city=杭州` 返回杭州的 Open-Meteo 实时天气，且设置分组在当前暗色玻璃弹窗中无横向溢出。
- 自动定位基于网络位置，VPN/代理环境下可能不准确；用户始终可以手动修改城市。
- 自动定位首次上线后确认 `ip-api.com` 在当前网络中持续连接超时。`server.js` 已改为优先使用 HTTPS `ipwho.is`，并以原 `ip-api.com` 为第二级兜底；单个定位源超时缩短为 4.5 秒。
- `/api/weather/ip-location` 已在重启后验证返回 `ok: true`，包含城市、坐标和时区；返回值不再携带公网 IP 字段。
