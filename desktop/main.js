...
let qqEmbeddedLoginState = null;
let qqEmbeddedLoginStartPromise = null;
...
const QQ_EMBEDDED_LOGIN_URL = 'https://xui.ptlogin2.qq.com/cgi-bin/xlogin?appid=716027609&daid=383&style=33&login_text=%E7%99%BB%E5%BD%95&hide_title_bar=1&hide_border=1&target=self&s_url=https%3A%2F%2Fgraph.qq.com%2Foauth2.0%2Flogin_jump&pt_3rd_aid=100497308&pt_feedback_link=https%3A%2F%2Fsupport.qq.com%2Fproducts%2F77942%3FcustomInfo%3D.appid100497308&theme=2';
const QQ_LOGIN_CHROME_MAJOR = String(process.versions.chrome || '148').split('.')[0] || '148';
const QQ_LOGIN_USER_AGENT = `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${QQ_LOGIN_CHROME_MAJOR}.0.0.0 Safari/537.36`;
const QQ_LOGIN_POPUP_HOSTS = new Set([
  'ptlogin2.qq.com',
  'xui.ptlogin2.qq.com',
  'ssl.ptlogin2.qq.com',
  'graph.qq.com',
...
function closeQQEmbeddedLogin() {
  const state = qqEmbeddedLoginState;
  qqEmbeddedLoginState = null;
  if (!state) return;
  state.closing = true;
  if (state.expiryTimer) clearTimeout(state.expiryTimer);
  if (state.children) {
    state.children.forEach((childWindow) => {
...
async function inspectQQEmbeddedFrames(state) {
  if (!state || !state.window || state.window.isDestroyed()) return null;
  const frames = state.window.webContents.mainFrame.framesInSubtree || [];
  let detected = null;
  for (const frame of frames) {
    if (!frame || frame.isDestroyed()) continue;
    let isQQLoginFrame = false;
    try {
      const frameHost = new URL(frame.url || '').hostname.toLowerCase();
      isQQLoginFrame = frameHost === 'ptlogin2.qq.com' || frameHost.endsWith('.ptlogin2.qq.com');
    } catch (_) {}
    try {
      const result = await frame.executeJavaScript(`
        (() => {
          ...
          const qr = images.find((img) => {
            const src = String(img.currentSrc || img.src || '');
            const idClass = String(img.id || '') + ' ' + String(img.className || '');
            const rect = img.getBoundingClientRect();
            return rect.width >= 70 && rect.height >= 70 && (/ptqrshow|qrcode|qr/i.test(src + ' ' + idClass));
          });
          const canvas = Array.from(document.querySelectorAll('canvas')).find((node) => {
            ...
            return rect.width >= 70 && rect.height >= 70 && /qr|code/i.test(idClass);
          });
          let data = '';
          try {
            if (qr && qr.complete && qr.naturalWidth && qr.naturalHeight) {
              const copy = document.createElement('canvas');
              copy.width = qr.naturalWidth;
              copy.height = qr.naturalHeight;
              copy.getContext('2d').drawImage(qr, 0, 0);
              data = copy.toDataURL('image/png');
            } else if (!qr && canvas) {
              data = canvas.toDataURL('image/png');
            }
          } catch (_) {}
          return {
            src: qr ? String(qr.currentSrc || qr.src || '') : '',
            data,
            expired: /二维码.{0,6}(失效|过期)|点击刷新/.test(text),
            scanned: /已扫码|已扫描|扫码成功|扫描成功|请在手机(?:上)?确认/.test(text),
          };
        })()
      `, true);
      if (!result) continue;
      detected = detected || result;
      if (isQQLoginFrame && result.scanned) state.scanned = true;
      if (isQQLoginFrame && result.expired) state.expired = true;
      if (isQQLoginFrame && (result.src || result.data)) return result;
    } catch (_) {}
  }
  return detected;
}

async function readQQEmbeddedQrImage(state, imageUrl) {
  if (!state || !state.cookieSession || !imageUrl) return '';
  if (/^data:image\//i.test(imageUrl)) return imageUrl;
  if (!/^https:\/\//i.test(imageUrl)) return '';
  const response = await state.cookieSession.fetch(imageUrl, {
    headers: {
      Referer: 'https://xui.ptlogin2.qq.com/',
      'User-Agent': QQ_LOGIN_USER_AGENT,
    },
  });
  if (!response.ok) throw new Error(`QQ_QR_IMAGE_HTTP_${response.status}`);
  const bytes = Buffer.from(await response.arrayBuffer());
  if (!bytes.length || bytes.length > 2 * 1024 * 1024) throw new Error('QQ_QR_IMAGE_INVALID');
  const contentType = String(response.headers.get('content-type') || 'image/png').split(';')[0];
  return `data:${/^image\//i.test(contentType) ? contentType : 'image/png'};base64,${bytes.toString('base64')}`;
}

function qqQrToken(qrsig) {
  let hash = 0;
  const text = String(qrsig || '');
  for (let i = 0; i < text.length; i++) hash += (hash << 5) + text.charCodeAt(i);
  return hash & 0x7fffffff;
}

async function createQQDirectQr(state) {
  const qrUrl = new URL('https://xui.ptlogin2.qq.com/ssl/ptqrshow');
  qrUrl.searchParams.set('appid', '716027609');
  ...
  qrUrl.searchParams.set('daid', '383');
  qrUrl.searchParams.set('pt_3rd_aid', '100497308');
  const response = await state.cookieSession.fetch(qrUrl.toString(), {
    headers: {
      Referer: QQ_EMBEDDED_LOGIN_URL,
      'User-Agent': QQ_LOGIN_USER_AGENT,
    },
  });
  if (!response.ok) throw new Error(`QQ_QR_DIRECT_HTTP_${response.status}`);
  const bytes = Buffer.from(await response.arrayBuffer());
  if (!bytes.length || bytes.length > 2 * 1024 * 1024) throw new Error('QQ_QR_DIRECT_IMAGE_INVALID');
  const qrsigCookie = (await state.cookieSession.cookies.get({ name: 'qrsig' }))[0];
  if (!qrsigCookie || !qrsigCookie.value) throw new Error('QQ_QR_DIRECT_SIG_MISSING');
  state.qrMode = 'direct';
  state.qrsig = qrsigCookie.value;
  return `data:image/png;base64,${bytes.toString('base64')}`;
}

function parseQQPtuiCallback(text) {
  const values = [];
  String(text || '').replace(/'((?:\\'|[^'])*)'/g, (_all, value) => {
    values.push(value.replace(/\\'/g, "'"));
    return _all;
  });
  return { code: Number(values[0]), redirect: values[2] || '', message: values[4] || '' };
}

async function pollQQDirectQr(state) {
  if (!state || state.qrMode !== 'direct' || !state.qrsig) return null;
  const pollUrl = new URL('https://ssl.ptlogin2.qq.com/ptqrlogin');
  pollUrl.searchParams.set('u1', 'https://graph.qq.com/oauth2.0/login_jump');
  pollUrl.searchParams.set('ptqrtoken', String(qqQrToken(state.qrsig)));
  ...
  pollUrl.searchParams.set('js_ver', '26030415');
  pollUrl.searchParams.set('js_type', '1');
  pollUrl.searchParams.set('login_sig', '');
  pollUrl.searchParams.set('pt_uistyle', '40');
  pollUrl.searchParams.set('aid', '716027609');
  pollUrl.searchParams.set('daid', '383');
  pollUrl.searchParams.set('pt_3rd_aid', '100497308');
  const response = await state.cookieSession.fetch(pollUrl.toString(), {
    headers: {
      Referer: QQ_EMBEDDED_LOGIN_URL,
      'User-Agent': QQ_LOGIN_USER_AGENT,
    },
  });
  if (!response.ok) throw new Error(`QQ_QR_POLL_HTTP_${response.status}`);
  const result = parseQQPtuiCallback(await response.text());
  if (result.code === 0 && /^https:\/\//i.test(result.redirect)) {
    state.scanned = true;
    state.qrMode = 'callback';
    state.window.loadURL(result.redirect).catch(() => {});
    return { ok: true, code: 802, message: '扫码已确认，正在同步 QQ 音乐授权…' };
  }
  if (result.code === 67) {
    state.scanned = true;
    return { ok: true, code: 802, message: '已扫码，请在手机确认登录…' };
  }
  if (result.code === 65 || result.code === 68) {
    state.expired = true;
    return { ok: false, code: 800, message: result.message || 'QQ 音乐二维码已过期，请刷新' };
  }
  return { ok: true, code: 801, message: '请使用 QQ 音乐 App 扫码' };
}

async function createQQEmbeddedLogin() {
  closeQQEmbeddedLogin();
  ...
  state.window = loginWindow;
  loginWindow.webContents.setUserAgent(QQ_LOGIN_USER_AGENT);
  loginWindow.webContents.setWindowOpenHandler(({ url }) => {
    let target = null;
    try { target = new URL(url); } catch (_) {}
    const host = target ? target.hostname.toLowerCase() : '';
    const trusted = target && target.protocol === 'https:' && (
      host === 'y.qq.com' || QQ_LOGIN_POPUP_HOSTS.has(host) || host.endsWith('.ptlogin2.qq.com')
    );
    if (!trusted) return { action: 'deny' };
    return {
      action: 'allow',
  ...
  loginWindow.webContents.on('did-create-window', (childWindow) => {
    ...
    state.children.add(childWindow);
    childWindow.webContents.setUserAgent(QQ_LOGIN_USER_AGENT);
    childWindow.hide();
    childWindow.on('closed', () => state.children.delete(childWindow));
  });
  loginWindow.on('closed', () => {
    if (qqEmbeddedLoginState === state && !state.closing) state.expired = true;
    state.window = null;
  });
  loginWindow.webContents.on('did-finish-load', () => {
    try {
      const loadedHost = new URL(loginWindow.webContents.getURL()).hostname.toLowerCase();
      if (loadedHost === 'ptlogin2.qq.com' || loadedHost.endsWith('.ptlogin2.qq.com')) return;
    } catch (_) {}
    loginWindow.webContents.executeJavaScript(`
      new Promise((resolve) => setTimeout(() => {
        ...
        const nodes = Array.from(document.querySelectorAll('a, button, [role="button"], [class*="login"], [class*="Login"]'));
        const target = nodes.filter(visible).find((node) => /^(登录|登陆|立即登录)$/.test((node.textContent || '').replace(/\\s+/g, '').trim())) ||
          nodes.filter(visible).find((node) => /登录|登陆/.test((node.textContent || '').trim()));
  ...
  try {
    const directImg = await createQQDirectQr(state);
    state.qrImage = directImg;
    state.readyAt = Date.now();
    state.expiryTimer = setTimeout(() => {
      if (qqEmbeddedLoginState === state) closeQQEmbeddedLogin();
    }, 125000);
    return { ok: true, img: directImg, expiresIn: 120, reused: false, fastPath: true };
  } catch (_) {
    state.qrMode = '';
    state.qrsig = '';
  }
  ...
  const loginAttempts = [
    { url: QQ_EMBEDDED_LOGIN_URL, waitMs: 8000 },
    { url: QQ_LOGIN_URL, waitMs: 16000 },
  ];
  for (let i = 0; i < loginAttempts.length; i++) {
    const attempt = loginAttempts[i];
    try {
      await loginWindow.loadURL(attempt.url);
    } catch (e) {
      lastLoadError = e;
      if (i < loginAttempts.length - 1) await new Promise(resolve => setTimeout(resolve, 500));
      continue;
    }

    const deadline = Date.now() + attempt.waitMs;
    while (Date.now() < deadline && qqEmbeddedLoginState === state && !state.expired) {
      const found = await inspectQQEmbeddedFrames(state);
      if (found && (found.src || found.data)) {
        const img = found.data || await readQQEmbeddedQrImage(state, found.src);
        if (img) {
          state.qrImage = img;
          state.readyAt = Date.now();
          state.expiryTimer = setTimeout(() => {
          ...
          return { ok: true, img, expiresIn: 120, reused: false };
        }
      }
  ...
  closeQQEmbeddedLogin();
  throw lastLoadError || new Error('QQ_QR_NOT_FOUND');
}

async function startQQEmbeddedLogin(options = {}) {
  const force = !!(options && options.force);
  const state = qqEmbeddedLoginState;
  if (!force && state && state.qrImage && state.window && !state.window.isDestroyed() && Date.now() - Number(state.readyAt || 0) < 105000) {
    return { ok: true, img: state.qrImage, expiresIn: Math.max(1, Math.ceil((120000 - (Date.now() - state.readyAt)) / 1000)), reused: true };
  }
  if (!force && qqEmbeddedLoginStartPromise) return qqEmbeddedLoginStartPromise;
  if (force) closeQQEmbeddedLogin();

  const promise = createQQEmbeddedLogin();
  qqEmbeddedLoginStartPromise = promise;
  try {
    return await promise;
  } finally {
    if (qqEmbeddedLoginStartPromise === promise) qqEmbeddedLoginStartPromise = null;
  }
}

async function checkQQEmbeddedLogin() {
  const state = qqEmbeddedLoginState;
  if (!state || !state.window || state.window.isDestroyed()) {
    return { ok: false, code: 800, message: 'QQ 音乐二维码已失效，请刷新' };
  }
  const cookie = await readQQLoginCookieHeader(state.cookieSession);
  if (qqCookieHasPlaybackLogin(cookie)) {
    closeQQEmbeddedLogin();
    return { ok: true, code: 803, cookie, playbackKeyReady: true, message: 'QQ 音乐登录成功' };
  }
  if (qqCookieHasLogin(cookie)) {
    state.scanned = true;
    if (!state.warmupStarted) {
      state.warmupStarted = true;
      state.window.loadURL('https://y.qq.com/n/ryqq/profile/like/song').catch(() => {});
    }
    if (Date.now() - state.createdAt > 45000) {
      closeQQEmbeddedLogin();
      return { ok: true, code: 803, cookie, partial: true, playbackKeyReady: false, message: 'QQ 账号已登录，播放授权仍在同步' };
    }
    return { ok: true, code: 802, message: '已扫码，正在同步 QQ 音乐播放授权…' };
  }
  if (state.qrMode === 'direct') {
    try {
      const directStatus = await pollQQDirectQr(state);
      if (directStatus && Number(directStatus.code) === 800) {
        closeQQEmbeddedLogin();
        return directStatus;
      }
      if (directStatus) return directStatus;
    } catch (_) {
      return { ok: true, code: state.scanned ? 802 : 801, message: state.scanned ? '已扫码，正在同步授权…' : '请使用 QQ 音乐 App 扫码' };
    }
  }
  await inspectQQEmbeddedFrames(state);
  if (state.expired || Date.now() - state.createdAt > 120000) {
    closeQQEmbeddedLogin();
    return { ok: false, code: 800, message: 'QQ 音乐二维码已过期，请刷新' };
  }
  return state.scanned
    ? { ok: true, code: 802, message: '已扫码，请在手机确认登录…' }
    : { ok: true, code: 801, message: '请使用 QQ 音乐 App 扫码' };
}
...
async function openQQMusicLoginWindow(owner) {
  ...
  return new Promise((resolve) => {
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      ...
      try { target = new URL(url); } catch (_) {}
      const host = target ? target.hostname.toLowerCase() : '';
      const isQQLoginPopup = target && target.protocol === 'https:' && (
        QQ_LOGIN_POPUP_HOSTS.has(host) || host.endsWith('.ptlogin2.qq.com')
      );
      if (isQQLoginPopup) {
        return {
          action: 'allow',
    ...
    loginWindow.webContents.on('did-create-window', (childWindow) => {
      ...
      childWindow.webContents.on('did-finish-load', checkCookies);
    });
      ...
      loginWindow.webContents.executeJavaScript(`
        new Promise((resolve) => setTimeout(() => {
          ...
          const candidates = nodes.filter((node) => {
            ...
            return isVisible(node) && (/(^|\\s)(登录|登陆|立即登录)(\\s|$)/.test(text) || /登录|登陆/.test(aria + title));
          });
          candidates.sort((a, b) => {
            const aText = (a.textContent || '').trim();
            const bText = (b.textContent || '').trim();
            return Number(aText.length > 12) - Number(bText.length > 12) || aText.length - bText.length;
          });
          const loginNode = candidates[0];
          if (loginNode) {
            loginNode.click();
            resolve('clicked:' + (loginNode.textContent || loginNode.getAttribute('aria-label') || '').trim().slice(0, 24));
          } else {
            resolve('not-found');
          }
        }, 1200));
...
async function createWindow() {
  ...
  global.__MINERADIO_QQ_LOGIN_BRIDGE__ = {
    start: startQQEmbeddedLogin,
    check: checkQQEmbeddedLogin,
    cancel: async () => {
      closeQQEmbeddedLogin();
      return { ok: true };
    },
  ...
  const isAllowedMediaRequest = (webContents, permission, details = {}) => {
    if (!mainWindow || mainWindow.isDestroyed() || webContents !== mainWindow.webContents || permission !== 'media') return false;
    const mediaTypes = Array.isArray(details.mediaTypes) ? details.mediaTypes.slice() : [];
    if (details.mediaType) mediaTypes.push(details.mediaType);
    return !mediaTypes.length || mediaTypes.every(type => type === 'audio' || type === 'video');
  };
  mainSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
    callback(isAllowedMediaRequest(webContents, permission, details));
  });
  mainSession.setPermissionCheckHandler((webContents, permission, _requestingOrigin, details) => {
    return isAllowedMediaRequest(webContents, permission, details);
  });
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    setTimeout(() => {
      try {
        const savedCookie = process.env.QQ_COOKIE_FILE && fs.existsSync(process.env.QQ_COOKIE_FILE)
          ? fs.readFileSync(process.env.QQ_COOKIE_FILE, 'utf8').trim()
          : '';
        if (!qqCookieHasPlaybackLogin(savedCookie)) startQQEmbeddedLogin().catch(() => {});
      } catch (_) {}
    }, 1200);
  });
