...
### 2026-07-13 - 晚晚语音对话点歌边界

- 用户认可/要求保留：首页「晚晚的音乐电台」支持麦克风语音输入，识别文字后必须复用原有 AI 聊天和插队播放链路；第一版只做 ASR，不做 TTS。
- 涉及文件：`public/index.html`、`server.js`、`desktop/main.js`。
- 关键参数/实现：渲染进程用 `MediaRecorder` 录制 `audio/webm;codecs=opus`，最长 60 秒、前端 10MB 自动停止；新增 `POST /api/ai/transcribe`，服务端 12MB 上限、30 秒超时，音频仅在内存 Buffer 中转发。支持硅基流动 `FunAudioLLM/SenseVoiceSmall` 和自定义 OpenAI 兼容端点。ASR 配置保存于 localStorage `mineradio-ai-asr-config-v1`。
- 禁止回退或改坏的点：不得换成 Electron 不可靠的 `webkitSpeechRecognition`；不得把 API Key、录音或识别样本写入项目；不得复制第二套 AI 发送/队列逻辑；麦克风权限处理不能破坏原有摄像头手势功能。

### 2026-07-10 - AI 电台必须使用真实天气选曲

- 用户认可/要求保留：用户询问“今天天气怎么样”时，晚晚必须回答当前城市的真实天气，并按当天天气推荐音乐。
- 涉及文件：`public/index.html`、`server.js`。
- 关键参数/实现：复用 Home 的 Open-Meteo 天气状态和城市设置；AI 对话前对超过 15 分钟的天气自动刷新；向 DeepSeek 传入城市、天气、温度、体感、湿度、风速和天气选曲情绪。
- 禁止回退或改坏的点：天气不可用时不允许 AI 猜测；不要为 AI 另建一套天气城市设置；天气选曲仍必须遵守“插入下一首、不清空原队列”。

### 2026-07-10 - AI 音乐电台首页融合边界

- 用户认可/要求保留：首页原“施工中”区域改为「晚晚的音乐电台」聊天区，AI 设置按钮必须沿用 Mineradio 全局暗色玻璃、细边框和高亮色体系；“展开播放器控制台”按钮继续保留。
- 涉及文件：`public/index.html`、`server.js`。
- 关键参数/实现：DeepSeek Key 由用户在 AI 区右上角设置弹窗填写，只保存在本机 localStorage；第一版不做 TTS。AI 推荐最多 3 首，网易云优先、QQ 音乐兜底；推荐歌曲保持顺序插入当前歌曲之后，立即播放第一首，播放结束后继续原队列。
- 禁止回退或改坏的点：不要清空用户原队列；不要另建第二套播放器；不要把 DeepSeek Key 写入仓库、安装包或日志；不要修改原有播放器控制台、歌词和视觉舞台核心。
...
### 2026-07-16 - AI 电台天气位置由用户选择

- 用户认可/要求保留：AI 音乐电台设置中可以选择天气城市，晚晚必须根据用户保存的位置回答具体实时天气并推荐音乐。
- 涉及文件：`public/index.html`、`AI_HANDOFF.md`、`docs/PROJECT_MEMORY.md`。
- 关键参数/实现：城市保存在 `localStorage` 的 `mineradio-weather-city`；设置页支持城市输入、常用城市建议和网络自动定位；保存后立即刷新 `/api/weather/radio`，AI 聊天仍通过 `context.weather` 使用 Open-Meteo 的城市、天气、温度、体感温度等事实数据。
- 禁止回退或改坏的点：不要重新固定为上海；不要在天气接口不可用时让 AI 猜测天气；自动定位不准时必须保留手动城市输入。
- 自动定位容错：定位服务必须采用多源降级，当前顺序为 HTTPS `ipwho.is` → `ip-api.com`，每个源 4.5 秒超时；不要恢复成单点 `ip-api.com`，也不要在接口响应或日志中返回公网 IP。
