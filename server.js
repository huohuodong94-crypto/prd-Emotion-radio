...
const WEATHER_IPWHO_LOCATION_URL = 'https://ipwho.is/';
const WEATHER_IP_API_LOCATION_URL = 'http://ip-api.com/json/';
...
function readBinaryRequestBody(req, maxBytes) {
  maxBytes = Number(maxBytes) || 12 * 1024 * 1024;
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      size += chunk.length;
      if (size > maxBytes) {
        const err = new Error('录音超过 12MB 限制');
        err.statusCode = 413;
        err.code = 'ASR_BODY_TOO_LARGE';
        fail(err);
        return;
      }
      chunks.push(chunk);
    ...
    req.on('error', err => {
      ...
      err.code = err.code || 'ASR_BODY_READ_FAILED';
      fail(err);
    });
  });
}
...
// ---------- AI 音乐电台 ----------
const AI_ASR_MAX_BYTES = 12 * 1024 * 1024;
const AI_ASR_TIMEOUT_MS = 30000;
...
const AI_TTS_MAX_BYTES = 12 * 1024 * 1024;
const AI_TTS_TIMEOUT_MS = 45000;
...
const SILICONFLOW_TTS_VOICE = 'FunAudioLLM/CosyVoice2-0.5B:claire';

function aiAsrHeader(req, name, maxLength) {
  return String(req.headers[name] || '').trim().slice(0, maxLength || 1000);
}

function normalizeAiAsrEndpoint(provider, baseURL) {
  if (provider === 'siliconflow') return SILICONFLOW_ASR_URL;
  if (provider !== 'custom') {
    throw Object.assign(new Error('不支持的语音识别服务商'), { statusCode: 400, code: 'ASR_PROVIDER_INVALID' });
  }
  if (!baseURL) {
    throw Object.assign(new Error('请填写 OpenAI 兼容语音识别端点'), { statusCode: 400, code: 'ASR_BASE_URL_REQUIRED' });
  }
  let target;
  try { target = new URL(baseURL); }
  catch (e) {
    throw Object.assign(new Error('语音识别端点地址无效'), { statusCode: 400, code: 'ASR_BASE_URL_INVALID' });
  }
  if (target.protocol !== 'https:' && target.protocol !== 'http:') {
    throw Object.assign(new Error('语音识别端点只支持 HTTP/HTTPS'), { statusCode: 400, code: 'ASR_BASE_URL_INVALID' });
  }
  target.search = '';
  target.hash = '';
  let pathname = target.pathname.replace(/\/+$/, '');
  if (!/\/audio\/transcriptions$/i.test(pathname)) {
    pathname += /\/v1$/i.test(pathname) ? '/audio/transcriptions' : '/v1/audio/transcriptions';
  }
  target.pathname = pathname.replace(/^\/?/, '/');
  return target.toString();
}

function aiAsrFileExtension(contentType) {
...
async function callAiRadioTranscription(req, audioBuffer) {
  const provider = aiAsrHeader(req, 'x-asr-provider', 40).toLowerCase() || 'siliconflow';
  const apiKey = aiAsrHeader(req, 'x-asr-key', 4096);
  const baseURL = aiAsrHeader(req, 'x-asr-base-url', 2048);
  const requestedModel = aiAsrHeader(req, 'x-asr-model', 300);
  if (!apiKey) {
    throw Object.assign(new Error('请先在 AI 设置里填写语音识别 API Key'), { statusCode: 400, code: 'ASR_KEY_REQUIRED' });
  }
  if (!audioBuffer || !audioBuffer.length) {
    throw Object.assign(new Error('没有收到录音内容'), { statusCode: 400, code: 'ASR_AUDIO_REQUIRED' });
  }
  if (audioBuffer.length > AI_ASR_MAX_BYTES) {
    throw Object.assign(new Error('录音超过 12MB 限制'), { statusCode: 413, code: 'ASR_BODY_TOO_LARGE' });
  }
  const endpoint = normalizeAiAsrEndpoint(provider, baseURL);
  const model = requestedModel || (provider === 'siliconflow' ? SILICONFLOW_ASR_MODEL : 'whisper-1');
  const contentType = String(req.headers['content-type'] || 'audio/webm').split(';')[0].trim() || 'audio/webm';
  const form = new FormData();
  form.append('file', new Blob([audioBuffer], { type: contentType }), `mineradio-voice.${aiAsrFileExtension(contentType)}`);
  ...
  try {
    response = await fetch(endpoint, {
      ...
      body: form,
      signal: AbortSignal.timeout(AI_ASR_TIMEOUT_MS),
    });
  } catch (err) {
    const timedOut = err && (err.name === 'TimeoutError' || err.name === 'AbortError');
    throw Object.assign(new Error(timedOut ? '语音识别超时，请稍后重试' : '无法连接语音识别服务'), {
      statusCode: timedOut ? 504 : 502,
      code: timedOut ? 'ASR_TIMEOUT' : 'ASR_UPSTREAM_FAILED',
    });
  }
  ...
  if (!response.ok) {
    const detail = payload && (payload.message || payload.error && (payload.error.message || payload.error.code));
    throw Object.assign(new Error(detail ? `语音识别失败：${String(detail).slice(0, 180)}` : `语音识别服务返回 ${response.status}`), {
      statusCode: 502,
      code: 'ASR_UPSTREAM_FAILED',
    });
  }
  const text = String(payload && (payload.text || payload.transcript || payload.result && payload.result.text) || '').trim();
  if (!text) {
    throw Object.assign(new Error('没有听清内容，请靠近麦克风再试一次'), { statusCode: 422, code: 'ASR_EMPTY' });
  }
  const duration = Number(payload && (payload.duration || payload.audio_duration));
  return { ok: true, text: text.slice(0, 800), ...(Number.isFinite(duration) ? { duration } : {}) };
}

async function callAiRadioSpeech(body) {
  const apiKey = String(body && body.apiKey || '').trim().slice(0, 4096);
  const input = String(body && body.input || '').replace(/\s+/g, ' ').trim().slice(0, 500);
  const model = String(body && body.model || SILICONFLOW_TTS_MODEL).trim().slice(0, 200) || SILICONFLOW_TTS_MODEL;
  const voice = String(body && body.voice || SILICONFLOW_TTS_VOICE).trim().slice(0, 300) || SILICONFLOW_TTS_VOICE;
  const speedValue = Number(body && body.speed);
  const speed = Number.isFinite(speedValue) ? Math.max(0.75, Math.min(1.25, speedValue)) : 1;
  if (!apiKey) {
    throw Object.assign(new Error('请先在 AI 设置里配置语音回复 API Key'), { statusCode: 400, code: 'TTS_KEY_REQUIRED' });
  }
  if (!input) {
    throw Object.assign(new Error('没有可朗读的回复内容'), { statusCode: 400, code: 'TTS_TEXT_REQUIRED' });
  }
  let response;
  try {
    response = await fetch(SILICONFLOW_TTS_URL, {
      method: 'POST',
      headers: {
      ...
      signal: AbortSignal.timeout(AI_TTS_TIMEOUT_MS),
    });
  } catch (err) {
    const timedOut = err && (err.name === 'TimeoutError' || err.name === 'AbortError');
    throw Object.assign(new Error(timedOut ? '语音回复生成超时，请稍后重试' : '无法连接语音回复服务'), {
      statusCode: timedOut ? 504 : 502,
      code: timedOut ? 'TTS_TIMEOUT' : 'TTS_UPSTREAM_FAILED',
    });
  }
  if (!response.ok) {
    const raw = await response.text();
    ...
    try { payload = raw ? JSON.parse(raw) : {}; } catch (e) {}
    const detail = payload && (payload.message || payload.error && (payload.error.message || payload.error.code));
    throw Object.assign(new Error(detail ? `语音回复失败：${String(detail).slice(0, 180)}` : `语音回复服务返回 ${response.status}`), {
      statusCode: 502,
  ...
  if (!audioBuffer.length) {
    throw Object.assign(new Error('语音回复服务没有返回音频'), { statusCode: 502, code: 'TTS_EMPTY' });
  }
  if (audioBuffer.length > AI_TTS_MAX_BYTES) {
    throw Object.assign(new Error('语音回复音频超过 12MB 限制'), { statusCode: 502, code: 'TTS_RESPONSE_TOO_LARGE' });
  }
  const contentType = String(response.headers.get('content-type') || 'audio/mpeg').split(';')[0].trim() || 'audio/mpeg';
  return { audioBuffer, contentType };
}

const AI_RADIO_SYSTEM_PROMPT = `你叫晚晚，是 Mineradio 的 24 小时 AI 音乐电台 DJ。
你温柔、克制、有陪伴感，不是客服，也不说教。用户疲惫、低落或失眠时，先接住情绪，再自然带出音乐；用户只想听歌时少说话。
根据用户输入、当前时间、实时天气、正在播放、后续队列、最近对话、最近播放和近期 AI 已推荐歌曲来选歌。默认优先符合用户口味的华语音乐；除非用户明确点名，否则不要推荐“近期 AI 已推荐”清单中的歌曲，也不要推荐当前播放或队列中已有的歌曲。
实时天气上下文由 Mineradio 天气接口提供，应视为事实。用户询问天气时，先简洁回答城市、天气和温度，再说明选曲方向，并必须推荐 1-3 首符合当前天气情绪的歌。如果上下文明确说天气不可用，必须如实说暂时无法获取，不得猜测。
明确点歌时推荐 1 首；场景推荐 2-3 首；普通闲聊可以不推荐。歌曲必须真实、歌名和歌手要准确，便于网易云或 QQ 音乐搜索。
只返回 JSON，不要 Markdown：
{"say":"自然简短的回应","play":[{"name":"歌曲名","artist":"歌手","reason":"简短理由"}],"reason":"选曲依据"}`;

function normalizeAiRadioText(value) {
  return String(value || '').toLowerCase().replace(/[\s·•・\-—_（）()《》【】\[\]]+/g, '');
}

function aiRadioReferenceBlocksWanted(reference, wanted) {
  const ref = normalizeAiRadioText(reference);
  const name = normalizeAiRadioText(wanted && wanted.name);
  const artist = normalizeAiRadioText(wanted && wanted.artist);
  if (!ref || !name || !ref.includes(name)) return false;
  return !artist || ref.includes(artist);
}

function aiRadioMessageExplicitlyRequests(message, wanted) {
  const normalizedMessage = normalizeAiRadioText(message);
  const name = normalizeAiRadioText(wanted && wanted.name);
  return name.length >= 2 && normalizedMessage.includes(name);
}

function aiRadioSongScore(song, wanted) {
  const songName = normalizeAiRadioText(song && song.name);
  const artist = normalizeAiRadioText(song && song.artist);
  const wantedName = normalizeAiRadioText(wanted && wanted.name);
  const wantedArtist = normalizeAiRadioText(wanted && wanted.artist);
  let score = 0;
  if (songName && wantedName && songName === wantedName) score += 8;
  else if (songName && wantedName && (songName.includes(wantedName) || wantedName.includes(songName))) score += 4;
  if (artist && wantedArtist && artist.includes(wantedArtist)) score += 5;
  if (/伴奏|翻唱|cover|live|现场|片段|铃声/i.test(String(song && song.name || ''))) score -= 3;
  return score;
}

async function resolveAiRadioSong(wanted) {
  const query = [wanted && wanted.name, wanted && wanted.artist].filter(Boolean).join(' ').trim();
  if (!query) return null;
  let neteaseSongs = [];
  try { neteaseSongs = await handleSearch(query, 6); }
  catch (err) { console.warn('[AiRadio] Netease search failed:', err.message); }
  const neteaseMatch = neteaseSongs
    .map(song => ({ song, score: aiRadioSongScore(song, wanted) }))
    .sort((a, b) => b.score - a.score)[0];
  if (neteaseMatch && neteaseMatch.song && neteaseMatch.score >= 4) {
    return { ...neteaseMatch.song, aiReason: String(wanted.reason || '') };
  }
  let qqSongs = [];
  try { qqSongs = await handleQQSearch(query, 6); }
  catch (err) { console.warn('[AiRadio] QQ search failed:', err.message); }
  const qqMatch = qqSongs
    .map(song => ({ song, score: aiRadioSongScore(song, wanted) }))
    .sort((a, b) => b.score - a.score)[0];
  if (qqMatch && qqMatch.song && qqMatch.score >= 4) {
    return { ...qqMatch.song, aiReason: String(wanted.reason || '') };
  }
  return null;
}

function extractAiRadioReply(text) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed && parsed.say) {
        return {
          say: String(parsed.say).slice(0, 500),
          reason: String(parsed.reason || '').slice(0, 300),
          play: (Array.isArray(parsed.play) ? parsed.play : []).slice(0, 3).map(item => ({
            name: String(item && item.name || '').trim().slice(0, 100),
            artist: String(item && item.artist || '').trim().slice(0, 100),
            reason: String(item && item.reason || '').trim().slice(0, 200),
...
async function callAiRadioDeepSeek(body) {
  const apiKey = String(body && body.apiKey || '').trim();
  const message = String(body && body.message || '').trim().slice(0, 800);
  if (!apiKey) throw Object.assign(new Error('请先在 AI 设置里填写 DeepSeek API Key'), { statusCode: 400, code: 'AI_KEY_REQUIRED' });
  if (!message) throw Object.assign(new Error('请输入想对晚晚说的话'), { statusCode: 400, code: 'AI_MESSAGE_REQUIRED' });
  ...
  const weatherText = weather && weather.available
    ? [
        weather.location || '当前城市',
        weather.label || '天气未知',
        Number.isFinite(Number(weather.temperature)) ? `${Number(weather.temperature)}℃` : '',
        Number.isFinite(Number(weather.apparentTemperature)) ? `体感 ${Number(weather.apparentTemperature)}℃` : '',
        Number.isFinite(Number(weather.humidity)) ? `湿度 ${Number(weather.humidity)}%` : '',
        Number.isFinite(Number(weather.windSpeed)) ? `风速 ${Number(weather.windSpeed)} km/h` : '',
        weather.moodTitle ? `天气选曲情绪：${weather.moodTitle}` : '',
        weather.moodTagline || '',
      ].filter(Boolean).join('；')
    : '暂时不可用（禁止猜测天气）';
  const contextText = [
    ...
    `实时天气：${weatherText}`,
    `当前播放：${String(context.current || '暂无')}`,
    `后续队列：${(Array.isArray(context.queue) ? context.queue : []).slice(0, 12).join('；') || '暂无'}`,
    `最近播放：${(Array.isArray(context.recent) ? context.recent : []).slice(0, 20).join('；') || '暂无'}`,
    `近期 AI 已推荐（7 天内冷却，除非用户明确点名否则禁止重复）：${(Array.isArray(context.recommended) ? context.recommended : []).slice(0, 20).join('；') || '暂无'}`,
  ].join('\n');
  const response = await fetch('https://api.deepseek.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1200,
      messages: [
        { role: 'system', content: AI_RADIO_SYSTEM_PROMPT },
        ...history,
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error(`DeepSeek 请求失败（${response.status}）${detail ? '：' + detail.slice(0, 160) : ''}`), { statusCode: 502, code: 'AI_UPSTREAM_FAILED' });
  }
  const payload = await response.json();
  const content = payload && payload.choices && payload.choices[0] && payload.choices[0].message && payload.choices[0].message.content;
  const reply = extractAiRadioReply(content);
  if (!reply) throw Object.assign(new Error('AI 返回格式异常，请重试'), { statusCode: 502, code: 'AI_REPLY_INVALID' });
  const blockedReferences = [
    String(context.current || ''),
    ...(Array.isArray(context.queue) ? context.queue : []).slice(0, 12),
    ...(Array.isArray(context.recommended) ? context.recommended : []).slice(0, 20),
  ].filter(Boolean);
  const playCandidates = reply.play.filter(wanted => {
    if (aiRadioMessageExplicitlyRequests(message, wanted)) return true;
    return !blockedReferences.some(reference => aiRadioReferenceBlocksWanted(reference, wanted));
  });
  const songs = [];
  const resolvedKeys = new Set();
  for (const wanted of playCandidates) {
    const resolved = await resolveAiRadioSong(wanted);
    const resolvedKey = resolved ? `${normalizeAiRadioText(resolved.name)}|${normalizeAiRadioText(resolved.artist)}` : '';
    if (resolved && resolvedKey && !resolvedKeys.has(resolvedKey)) {
      resolvedKeys.add(resolvedKey);
      songs.push(resolved);
    }
  }
  return { ok: true, reply, songs };
}
...
function requestText(targetUrl, opts, body) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutMs = Number.isFinite(requestedTimeout) ? Math.max(1000, Math.min(30000, requestedTimeout)) : 10000;
...
async function fetchIpWeatherLocation() {
  const sources = [
    {
      id: 'ipwho',
      load: async () => {
        const u = new URL(WEATHER_IPWHO_LOCATION_URL);
        u.searchParams.set('fields', 'success,message,country,region,city,latitude,longitude,timezone');
        const body = await requestJson(u.toString(), { headers: { 'User-Agent': UA }, timeoutMs: 4500 });
        if (!body || body.success === false || !Number.isFinite(Number(body.latitude)) || !Number.isFinite(Number(body.longitude))) {
          throw new Error(body && body.message || 'IPWHO_LOCATION_FAILED');
        }
        const timezone = typeof body.timezone === 'string' ? body.timezone : body.timezone && body.timezone.id;
        return {
          provider: 'ipwho',
    ...
    },
    {
      id: 'ip-api',
      load: async () => {
        const u = new URL(WEATHER_IP_API_LOCATION_URL);
        u.searchParams.set('fields', 'status,message,country,regionName,city,lat,lon,timezone');
        u.searchParams.set('lang', 'zh-CN');
        const body = await requestJson(u.toString(), { headers: { 'User-Agent': UA }, timeoutMs: 4500 });
        if (!body || body.status !== 'success' || !Number.isFinite(Number(body.lat)) || !Number.isFinite(Number(body.lon))) {
          throw new Error(body && body.message || 'IP_API_LOCATION_FAILED');
        }
        return {
          provider: 'ip-api',
          city: body.city || WEATHER_DEFAULT_LOCATION.name,
  ...
  for (const source of sources) {
    try {
      ...
      failures.push(`${source.id}:${err && err.message || 'failed'}`);
      console.warn(`[WeatherIpLocation] ${source.id} unavailable:`, err && err.message || err);
    }
  }
  const error = new Error('定位服务暂时不可用');
  error.code = 'IP_LOCATION_PROVIDERS_FAILED';
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/ai/chat') {
    ...
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await callAiRadioDeepSeek(body));
    } catch (err) {
      console.error('[AiRadio]', err.code || err.message);
      sendJSON(res, { ok: false, error: err.code || 'AI_RADIO_FAILED', message: err.message || 'AI 电台暂时不可用' }, err.statusCode || 500);
    }
    return;
  }

  if (pn === '/api/ai/transcribe') {
    if (req.method !== 'POST') {
      sendJSON(res, { ok: false, code: 'METHOD_NOT_ALLOWED', message: '只支持 POST 请求' }, 405);
      return;
    }
    try {
      const contentLength = Number(req.headers['content-length'] || 0);
      if (contentLength > AI_ASR_MAX_BYTES) {
        throw Object.assign(new Error('录音超过 12MB 限制'), { statusCode: 413, code: 'ASR_BODY_TOO_LARGE' });
      }
      const audioBuffer = await readBinaryRequestBody(req, AI_ASR_MAX_BYTES);
      sendJSON(res, await callAiRadioTranscription(req, audioBuffer));
    } catch (err) {
      console.error('[AiRadioAsr]', err.code || err.message);
      sendJSON(res, { ok: false, code: err.code || 'ASR_FAILED', message: err.message || '语音识别暂时不可用' }, err.statusCode || 500);
    }
    return;
  }
  ...
  if (pn === '/api/ai/speech') {
    ...
    try {
      const body = await readRequestBody(req);
      const speech = await callAiRadioSpeech(body);
      res.writeHead(200, {
        'Content-Type': speech.contentType,
        'Content-Length': speech.audioBuffer.length,
        'Access-Control-Allow-Origin': '*',
        'Cache-Control': 'no-store, no-cache, must-revalidate, proxy-revalidate',
        'Pragma': 'no-cache',
        'Expires': '0',
      });
      res.end(speech.audioBuffer);
    } catch (err) {
      console.error('[AiRadioTts]', err.code || err.message);
      sendJSON(res, { ok: false, code: err.code || 'TTS_FAILED', message: err.message || '语音回复暂时不可用' }, err.statusCode || 500);
    }
    return;
  }
  ...
  // ---------- 歌曲URL ----------
  if (pn === '/api/qq/login/qr/create') {
    const bridge = global.__MINERADIO_QQ_LOGIN_BRIDGE__;
    if (!bridge || typeof bridge.start !== 'function') {
      sendJSON(res, { ok: false, code: 'QQ_QR_DESKTOP_REQUIRED', message: '请从 Mineradio 桌面应用打开 QQ 音乐二维码' }, 501);
      return;
    }
    try {
      const result = await bridge.start({ force: url.searchParams.get('force') === '1' });
      sendJSON(res, result);
    } catch (err) {
      console.error('[QQLoginQrCreate]', err.message || err);
      sendJSON(res, { ok: false, code: err.code || 'QQ_QR_CREATE_FAILED', message: 'QQ 音乐二维码生成失败，请重试' }, 502);
    }
    return;
  }
  ...
  if (pn === '/api/qq/login/qr/check') {
    ...
    if (!bridge || typeof bridge.check !== 'function') {
      sendJSON(res, { ok: false, code: 'QQ_QR_DESKTOP_REQUIRED', message: '当前环境不支持 QQ 音乐扫码登录' }, 501);
      return;
    }
    try {
      const result = await bridge.check();
      if (Number(result && result.code) === 803 && result.cookie) {
        const normalized = normalizeQQCookieInput(result.cookie);
        const obj = parseCookieString(normalized);
        if (!qqCookieUin(obj) || !qqCookieMusicKey(obj)) {
          sendJSON(res, { ok: false, code: 'QQ_QR_COOKIE_INVALID', message: '已确认扫码，但没有拿到 QQ 音乐登录凭证' }, 502);
          return;
        }
        saveQQCookie(normalized);
        const info = await getQQLoginInfo();
        sendJSON(res, { ...result, ...info, cookie: undefined, saved: true });
        return;
      }
      sendJSON(res, result);
    } catch (err) {
      console.error('[QQLoginQrCheck]', err.message || err);
      sendJSON(res, { ok: false, code: err.code || 'QQ_QR_CHECK_FAILED', message: 'QQ 音乐扫码状态检查失败' }, 502);
    }
    return;
  }

  if (pn === '/api/qq/login/qr/cancel') {
    const bridge = global.__MINERADIO_QQ_LOGIN_BRIDGE__;
    if (bridge && typeof bridge.cancel === 'function') await bridge.cancel();
